...
> 来源：[官方文档](https://uniapp.dcloud.net.cn/tutorial/vue3-basics.html)
...
-   现在 `template` 是一级节点，用于写tag组件， `script` 和 `style` 是并列的一级节点，也就是有3个一级节点。这个叫[vue单文件组件规范sfc](https://uniapp.dcloud.net.cn/tutorial/vue3-components.html)。
...
```
	...
	<script>
		export default {
			...
			methods: {
				add() {
					this.sum = parseInt(this.a) + parseInt
				}
			}
		}
	</script>
...
```
	...
	<script>
		export default {
			...
			methods: {
				add() {
					this.sum = parseInt(this.obj.a) + parseInt
				}
			}
		}
	</script>
