...
2. `el-descriptions` 用 `:column="2"` + `class="app-desc"`（**不要加 border**）；每个 `el-descriptions-item` 加 `label-class-name="my-label"`，且 `label` 文本末尾**必须带中文全角冒号 `：`**（如 `label="统一社会信用代码："`，切勿用英文 `:` 或漏写）
